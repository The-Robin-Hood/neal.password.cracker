...
helper.periodicTableKeys = Object.keys(helper.periodicTable);
...
function setThePassword(
  ...
  let newPasswrd = existingPasswd;
  newPasswrd += `<span>----------</span>`;
  for (let i in str) {
    ...
    passwd += str[i];
    newPasswrd += `<span style="font-family: ${fontFace}; font-size: ${fontSize}px; font-weight: ${fontWeight}">${str[i]}</span>`;
...
async function rule11() {
  ...
  for (let i in wordle) {
    if (
      !helper.periodicTableKeys.includes(wordle[i].toUpperCase()) &&
      !capitalize
    ) {
...
function rule12() {
  let randomOnePeriodicTableKey =
    helper.periodicTableKeys[
      Math.floor(Math.random() * helper.periodicTableKeys.length)
    ];
...
function rule13() {
  ...
  const moonPhaseIndex = Math.floor((dayOfMonth / 8) % 8);
  const currentMoonEmoji = moonPhases[moonPhaseIndex];
  setThePassword(passwd, currentMoonEmoji);
}
...
function rule14() {
  ...
  if (embedLinks.includes(url) || links.includes(url)) {
    ...
    setThePassword(passwd, title.trim().replace(" ", ""));
  }
}
  ...
  setInterval(() => {
    ...
    rules.forEach((e) =>
      e.src.includes("error")
        ? (RULES[
            e.parentNode.innerText.trim().replace(" ", "").replace("R", "r")
          ] = false)
        : (RULES[
            e.parentNode.innerText.trim().replace(" ", "").replace("R", "r")
          ] = true)
