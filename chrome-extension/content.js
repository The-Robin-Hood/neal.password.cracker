...
const date = new Date();
const todayDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(
  2,
  "0"
...
function rule5() {
  ...
  if (currentSum < 25) {
    ...
    passwrdBox.innerHTML = passwrdBox.innerHTML.replace(
      numberToBeReplaced,
      numbertobereplacedwith
    );
  }
}
...
async function rule11() {
  ...
  const response = await fetch(
    "https://neal.fun/api/password-game/wordle?date=" + todayDate
  );
...
function rule18() {
  ...
  if (difference > 0) {
    ...
    while (
      contains(helper.periodicTableKeys[difference - 1], [
        "I",
        "V",
        "X",
        "L",
    ...
    while (totalAtomicValue > 200) {
      let max = Math.max(
        ...periodicTableSymbols.map((e) => helper.periodicTable[e])
      );
